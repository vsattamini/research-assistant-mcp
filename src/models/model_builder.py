...
class OpenAIProvider(BaseLLMProvider):
    ...
    def create_client(self, config: ModelConfig) -> Any:
        ...
        return OpenAI(api_key=api_key, max_retries=config.retry_attempts)
...
class OllamaProvider(BaseLLMProvider):
    ...
    def create_client(self, config: ModelConfig) -> Any:
        ...
        return ChatOllama(
            model=config.model_name, base_url=base_url, temperature=config.temperature
        )
...
class ModelBuilder:
    ...
    def with_provider(self, provider: str) -> "ModelBuilder":
        ...
        if provider not in self._providers:
            raise ValueError(
                f"Unsupported provider: {provider}. Available: {list(self._providers.keys())}"
            )
    ...
    def build(self) -> "ModelBuilder":
        ...
        try:
            ...
            self._is_built = True
            logger.info(
                f"Model built successfully with provider: {self._config.provider}"
            )
            return self
    ...
    def run(self, prompt: Optional[str] = None) -> str:
        ...
        if prompt is None:
            ...
            if prompt is None:
                raise ValueError(
                    "No prompt provided. Either pass a prompt to run() or set user_prompt()"
                )
...
def create_anthropic_model(
    model_name: str = "claude-3-sonnet-20240229", **kwargs
) -> ModelBuilder:

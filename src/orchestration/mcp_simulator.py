...
class MCPSimulator:
    ...
    def create_session(self, question: str) -> str:
        ...
        session = ResearchSession(session_id=session_id, original_question=question)
    ...
    def high_level_plan(self, question: str) -> List[ResearchTask]:
        ...
        if TaskPlannerTool is not None:
            try:
                ...
                logger.info(
                    "Planned %d tasks for question using TaskPlannerTool", len(tasks)
                )
                return tasks
            except Exception as e:
                logger.warning(
                    "TaskPlannerTool failed: %s. Falling back to prompt planning.", e
                )
        ...
        try:
            ...
            for i, task_data in enumerate(tasks_data):
                ...
                logger.info(
                    "Planned %d tasks for question using fallback planning", len(tasks)
                )
                return tasks
        except Exception as e:
    ...
    def _execute_search_task(
        ...
        if self.search_coordinator is None:
            logger.warning(
                "Search coordinator not available, falling back to basic search"
            )
        ...
        try:
            ...
            search_result = self.search_coordinator.simple_search(
                query=session.original_question, focus=task.description
            )
                    ...
                    web_details.append(
                        {
                            ...
                            "content_snippet": getattr(
                                r, "content", "No content preview"
                            ),  # Complete content
                            "source_domain": (
                                r.url.split("/")[2]
                        ...
                        "details": {
                            ...
                            "unique_domains": len(
                                set(
                                    [
                                        r.url.split("/")[2]
                                        for r in search_result.web_results
                                        if hasattr(r, "url") and r.url
                                    ]
                                )
                            ),
                    ...
                    arxiv_papers.append(
                        {
                            ...
                            "arxiv_id": (
                                p.metadata.get("arxiv_id", "Unknown")
                                if p.metadata
                                else "Unknown"
                            ),
                            "url": p.url,
                ...
                subtasks.append(
                    {
                        ...
                        "details": {
                            ...
                            "results_retrieved": min(
                                len(search_result.arxiv_results), 5
                            ),
                            "pdfs_available": processing_details.get(
                                "pdfs_available",
                                len(
                                    [
                                        p
                                        for p in search_result.arxiv_results
                                        if p.pdf_url
                                    ]
                                ),
                            ),
                            "pdfs_downloaded": processing_details.get(
                                "pdfs_downloaded",
                                len(
                                    [
                                        p
                            ...
                            "insights_extracted": processing_details.get(
                ...
                subtasks.append(
                    {
                        ...
                        "details": {
                            "processed_sources": len(
                                search_result.standardized_results
                            ),
            ...
            return {
                ...
                "total_results": len(search_result.web_results)
                + len(search_result.arxiv_results),
    ...
    def _execute_extract_task(
        ...
        if search_tasks and all(
            t.output_data.get("results_processed")
            for t in search_tasks
            if t.output_data
        ):
            logger.info(
                "All search results are already processed; skipping extraction task %s",
                task.id,
            )
            return {
                "skipped": True,
                ...
                "subtasks": [
                    {
                        ...
                        "details": {
                            ...
                            "processed_search_tasks": len(
                                [
                                    t
                                    for t in search_tasks
                                    if t.output_data.get("results_processed")
                                ]
                            ),
                            "timestamp": datetime.now().isoformat(),
                        },
                ...
                detailed_web_sources.append(
                    {
                        ...
                        "domain": (
                            s.get("url", "").split("/")[2]
                            if s.get("url") and len(s.get("url", "").split("/")) > 2
                            else "Unknown"
                        ),
                    }
                )
    ...
    def _execute_summarize_task(
        ...
        logger.info(
            "Skipping summarization task %s because synthesis handles aggregation",
            task.id,
        )
        return {
            "skipped": True,
    ...
    def _execute_synthesize_task(
        ...
        for prev in session.tasks:
            if prev.task_type == TaskType.SEARCH and prev.output_data:
                standardized_blocks.extend(
                    prev.output_data.get("standardized_results", []) or []
                )
    ...
    def _execute_report_task(
        ...
        synthesis_tasks = [
            t for t in completed_tasks if t.task_type == TaskType.SYNTHESIZE
        ]
        ...
        sections_count = len(
            [line for line in report_result.split("\n") if line.strip().startswith("#")]
        )
        subtasks.append(
            {
    ...
    def run_research(self, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.vector_db is not None:
                ...
                if retrieve_results:
                    ...
                    if high_quality_results:
                        ...
                        session.reasoning_steps.append(
                            "No high-quality cached knowledge found (below 70% threshold)"
                        )
            # Plan tasks
            ...
            for task in tasks:
                ...
                session.reasoning_steps.append(
                    f"Completed {task.task_type.value}: {task.description}"
                )

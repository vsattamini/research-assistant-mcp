...
class DocumentProcessor:
    ...
    def process_text(self, text: str, document_id: str = None) -> DocumentAnalysis:
        ...
        logger.info(f"Processed document {document_id}: {len(sections)} sections, {len(key_findings)} findings")
        return analysis
    ...
    def _generate_summary(self, text: str) -> str:
        ...
        for sentence in sentences:
            ...
            keywords = ['important', 'significant', 'key', 'major', 'finding', 'result', 'conclusion']
    ...
    def _calculate_section_importance(self, content: List[str]) -> float:
        ...
        # Score based on keywords
        important_words = ['important', 'significant', 'key', 'major', 'finding', 'result', 'conclusion', 'evidence']
        score += sum(1 for word in important_words if word in text.lower()) / len(important_words)
    ...
    def _identify_common_themes(self, findings: List[str]) -> List[str]:
        ...
        word_counts = {}
        for word in words:
            if word not in ['this', 'that', 'with', 'from', 'they', 'have', 'been', 'were', 'will', 'said']:
        ...
        for i, finding1 in enumerate(all_findings):
            for finding2 in all_findings[i+1:]:
                ...
                if has_positive and has_negative:
                    contradictions.append(f"Contradiction between findings: {finding1[:50]}... vs {finding2[:50]}...")
        
        return contradictions[:5]
    
    def _collect_supporting_evidence(self, documents: List[DocumentAnalysis]) -> List[str]:
...
def process_document_text(text: str, document_id: str = None) -> DocumentAnalysis:
    """
    ...
    return processor.process_text(text, document_id)
...
if __name__ == "__main__":
    ...
    sample_text = """
    Research on Renewable Energy Costs
    
    Introduction:
    This study examines the cost trends of renewable energy technologies over the past decade.
    
    Methodology:
    We analyzed data from 2014 to 2024, covering solar, wind, and battery storage costs.
    
    Results:
    Our findings show that solar energy costs decreased by 89% between 2014 and 2024.
    Wind energy costs declined by 70% during the same period.
    Battery storage costs fell by 97%, making renewable energy more competitive.
    
    Conclusion:
    Renewable energy is now cost-competitive with fossil fuels in most markets.
    Further research is needed to understand regional cost variations.
    """
    
    processor = DocumentProcessor()
    analysis = processor.process_text(sample_text, "renewable_energy_study")
    
    print(f"Document: {analysis.title}")
    print(f"Summary: {analysis.summary}")
    print(f"Key Findings: {len(analysis.key_findings)} found")
    print(f"Statistics: {len(analysis.statistics)} found")
    print(f"Sections: {len(analysis.sections)} found") 
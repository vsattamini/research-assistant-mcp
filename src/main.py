...
def research_assistant_streaming(question: str, show_reasoning: bool = True):
    ...
    try:
        ...
        if cached and (1 - cached[0]["distance"]) >= 0.9 and cached[0]["metadata"].get("answer"):
        ...
        if mcp_simulator.vector_db is not None:
            ...
            if retrieve_results:
                ...
                for i, result in enumerate(retrieve_results[:3], 1):  # Show top 3
                    ...
                    title = result.get("metadata", {}).get("title", f"Cached Result {i}")
                    similarity_details.append(f"   • **Result {i}**: {title} (Similarity: {similarity_score:.1f}%)")
                ...
                if high_quality_results:
                    ...
                    retrieval_details = f"\n📚 **KNOWLEDGE RETRIEVAL**\n\n✅ Found {len(retrieve_results)} total cached documents\n"
                    retrieval_details += f"▶ Cache threshold for usage: >70% similarity required\n"
                    retrieval_details += f"▶ High-quality matches: 0 documents above threshold\n"
                    retrieval_details += f"▶ Top matches found:\n" + "\n".join(similarity_details)
        ...
        for i, task in enumerate(tasks, 1):
            task_plan_details += f"   {i}. {task.task_type.value.upper()}: {task.description}\n"
        ...
        for i, task in enumerate(tasks, 1):
            ...
            task_complete_details += f"▶ Completed at: {datetime.now().strftime('%H:%M:%S')}\n"
            ...
            if task.task_type.value == "search":
                ...
                task_complete_details += f"▶ Sources found: {len(search_info)} total sources\n"
                if search_info:
                    web_count = len([s for s in search_info if s.get("type") == "web"])
                    academic_count = len([s for s in search_info if s.get("type") == "academic"])
                    task_complete_details += f"▶ Web sources: {web_count}, Academic papers: {academic_count}\n"
            
            elif task.task_type.value == "extract":
                task_complete_details += f"▶ Key insights extracted from search results\n"
                ...
                if isinstance(result, dict) and "report" in result:
                    ...
                    task_complete_details += f"▶ Report length: {len(session.final_answer)} characters\n"
            ...
            if isinstance(result, dict) and "subtasks" in result:
                ...
                for j, subtask in enumerate(result["subtasks"], 1):
                    ...
                    task_complete_details += f"   • {subtask.get('description', 'No description')}\n"
                    ...
                    if details:
                        if subtask.get("type") == "web_search":
                            ...
                            all_results = details.get("top_results", [])  # Show ALL results
                            if all_results:
                                task_complete_details += f"   • ALL Web Results Retrieved ({len(all_results)} total):\n"
                                for result_item in all_results:
                                    title = result_item.get("title", "Unknown")  # Complete title
                                    ...
                                    task_complete_details += f"       - Domain: {domain}\n"
                                    task_complete_details += f"       - Relevance: {relevance:.3f}\n"
                                    task_complete_details += f"       - URL: {url}\n"
                                    if result_item.get("content_snippet"):
                                        content = result_item["content_snippet"]  # Complete content
                                        task_complete_details += f"       - Content: {content}\n"
                                    task_complete_details += f"\n"
                            ...
                            papers = details.get("papers_detailed", [])  # Show ALL papers
                            if papers:
                                task_complete_details += f"   • ALL ArXiv Papers Retrieved ({len(papers)} total):\n"
                                for paper in papers:
                                    title = paper.get("title", "Unknown")  # Complete title
                                    authors = ", ".join(paper.get("authors", []))  # ALL authors
                                    categories = ", ".join(paper.get("categories", []))  # ALL categories
                                    arxiv_id = paper.get("arxiv_id", "Unknown")
                                    published = paper.get("published_date", "Unknown")[:10]
                                    url = paper.get("url", "No URL")
                                    pdf_url = paper.get("pdf_url", "No PDF URL")
                                    
                                    task_complete_details += f"     • [{paper.get('rank', '?')}] {title}\n"
                                    task_complete_details += f"       - Authors: {authors}\n"
                                    task_complete_details += f"       - ArXiv ID: {arxiv_id}\n"
                                    task_complete_details += f"       - Published: {published}\n"
                                    task_complete_details += f"       - Categories: [{categories}]\n"
                                    task_complete_details += f"       - URL: {url}\n"
                                    task_complete_details += f"       - PDF URL: {pdf_url}\n"
                                    ...
                                    if paper.get("abstract_snippet"):
                                        abstract = paper["abstract_snippet"]  # Complete abstract
                                        task_complete_details += f"       - Abstract: {abstract}\n"
                                    task_complete_details += f"\n"
                            ...
                            web_sources = details.get("detailed_web_sources", [])  # Show ALL
                            if web_sources:
                            ...
                            if details.get("content_preview"):
                                complete_content = details["content_preview"]  # Complete content, no truncation
                            ...
                            task_complete_details += f"   • Statistics Found: {stats_count}\n"
                            if stats_count > 0:
                                actual_stats = details.get('actual_statistics', [])  # Show ALL statistics
                            ...
                            task_complete_details += f"   • Findings Found: {findings_count}\n"
                            if findings_count > 0:
                                actual_findings = details.get('actual_findings', [])  # Show ALL findings
                                task_complete_details += f"   • Extraction Method: {details.get('extraction_method', 'Unknown')}\n"
                                if actual_findings:
                                    task_complete_details += f"   • ALL Findings Found ({len(actual_findings)} total):\n"
                                    for finding in actual_findings:
                                        finding_text = finding  # Complete finding, no truncation
                                        task_complete_details += f"     • {finding_text}\n"
                            else:
                                task_complete_details += f"   • Note: {details.get('note', 'No additional info')}\n"
                        ...
                        # Generic details for other subtask types
                            ...
                            key_details = {k: v for k, v in details.items() if k not in ['timestamp', 'content_preview'] and not k.endswith('_count') and not k.endswith('_used')}
                            for key, value in list(key_details.items())[:2]:  # Show top 2 details
        ...
        for task in tasks:
            ...
            task_summaries.append(f"{status_emoji} **{task.task_type.value.title()}**: {task.description}")
        ...
        response_parts.append(f"- **Completed**: {len([t for t in tasks if t.status.value == 'completed'])}")
        response_parts.append(f"- **Failed**: {len([t for t in tasks if t.status.value == 'failed'])}")
...
def create_interface():
    with gr.Blocks(title="Research Assistant", theme=gr.themes.Soft()) as demo:
        ...
        gr.Markdown("Ask me complex research questions and I'll help you find comprehensive answers using AI-powered MCP workflow with **real-time progress updates**!")
        ...
        with gr.Accordion("🧠 Detailed Reasoning Steps", open=False) as reasoning_accordion:
        ...
        def stream_research(question, show_reasoning):
            ...
            for status_update, reasoning_update in research_assistant_streaming(question, show_reasoning):

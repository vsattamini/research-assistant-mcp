...
class CSVAnalysisTool:
    ...
    def __init__(self, model_builder: Optional[ModelBuilder] = None):
        ...
        # Paths for CSV data
        self.csv_dir = "/home/vlofgren/Documents/Projects/research-assistant-mcp/data/csvs"
    ...
    def _load_csv_safely(self, filepath: str, max_rows: int = 10000) -> Optional[pd.DataFrame]:
        """
            ...
            for sep in separators_to_try:
                try:
                    ...
                    if len(sample.columns) > 1:  # Valid CSV structure
                        logger.info(f"CSV structure preview for {os.path.basename(filepath)} (encoding: {encoding}, sep: '{sep}'): {sample.columns.tolist()}")
                        
                        # Load full dataset with row limit
                        df = pd.read_csv(filepath, nrows=max_rows, encoding=encoding, sep=sep,
                                       on_bad_lines='skip', low_memory=False)
                        logger.info(f"Loaded {len(df)} rows from {os.path.basename(filepath)} with {encoding} encoding and '{sep}' separator")
                        return df
                    
                except UnicodeDecodeError:
                    logger.debug(f"Failed to load {os.path.basename(filepath)} with {encoding} encoding")
                    break  # Try next encoding
                except Exception as e:
                    logger.debug(f"Failed with {encoding} encoding and '{sep}' separator: {e}")
                    continue
    ...
    def _perform_basic_analysis(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if numeric_cols:
            ...
            analysis["correlations"] = df[numeric_cols].corr().to_dict() if len(numeric_cols) > 1 else {}
    ...
    def _extract_key_insights(self, df: pd.DataFrame, analysis: Dict[str, Any]) -> List[str]:
        """
        ...
        if "numeric_summary" in analysis:
            ...
            insights.append(f"Contains {len(numeric_cols)} numeric columns: {', '.join(numeric_cols[:3])}{'...' if len(numeric_cols) > 3 else ''}")
        ...
        if "categorical_summary" in analysis:
            ...
            for col, info in list(cat_summary.items())[:2]:
                ...
                if info["most_frequent"]:
                    insights.append(f"Most frequent value in '{col}': {info['most_frequent']}")
        
        return insights
    
    def _generate_summary(self, dataset_name: str, query: str, analysis: Dict[str, Any], 
...
"""
        ...
        for filename, description in relevant_datasets[:max_datasets]:
            ...
            try:
                ...
                result = AnalysisResult(
                    ...
                    statistics=analysis,
                    summary=summary,
                    relevant_columns=df.columns.tolist()[:10],  # Limit to first 10 columns
                ...
                results.append({
                    ...
                    "statistics": {
                        ...
                        "data_types": len(set(result.statistics["data_types"].values())),
                        "missing_data_pct": sum(result.statistics["missing_values"].values()) / 
                                          (result.statistics["shape"]["rows"] * result.statistics["shape"]["columns"]) * 100
                    },
    ...
    def _generate_overall_summary(self, query: str, results: List[Dict[str, Any]]) -> str:
        ...
        summary += "Each dataset provides complementary insights for understanding this topic."
        return summary

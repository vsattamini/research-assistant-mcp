...
class IntelligentSearchPlanner:
        ...
        self._planning_tool = {
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "required": ["primary_domain", "search_strategies", "search_terms", 
                               "academic_suitable", "arxiv_suitable", "focus_areas", "reasoning"]
                }
            }
        }
        ...
        self._validation_tool = {
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "required": ["relevance_score", "quality_score", "credibility_score", 
                               "completeness_score", "validation_notes"]
                }
            }
        }
        ...
        try:
            ...
            if hasattr(message, 'tool_calls') and message.tool_calls:
                ...
                return SearchPlan(
                    ...
                    search_strategies=[SearchStrategy(s) for s in data["search_strategies"]],
    ...
    def _basic_fallback(self, question: str) -> SearchPlan:
        ...
        arxiv_keywords = ['algorithm', 'computer', 'software', 'physics', 'mathematics', 
                         'machine learning', 'ai', 'artificial intelligence', 'quantum',
                         'statistics', 'probability', 'neural network']
        arxiv_suitable = any(keyword in question_lower for keyword in arxiv_keywords)
        
        # Determine domain
        if any(word in question_lower for word in ['economy', 'economic', 'financial', 'market']):
            domain = SearchDomain.ECONOMICS
        elif any(word in question_lower for word in ['health', 'medical', 'disease', 'treatment']):
            domain = SearchDomain.MEDICINE
        elif any(word in question_lower for word in ['technology', 'tech', 'software', 'computer']):
            domain = SearchDomain.TECHNOLOGY
        elif any(word in question_lower for word in ['recent', 'latest', 'current', 'news']):
    ...
    def validate_search_results(self, question: str, results_summary: str, 
                              search_plan: SearchPlan) -> SearchValidation:
        """
    ...
    def _structured_validation(self, question: str, results_summary: str, 
                             search_plan: SearchPlan) -> SearchValidation:
    ...
    def _fallback_validation(self, question: str, results_summary: str, 
                           search_plan: SearchPlan) -> SearchValidation:
        ...
        try:
            ...
            return SearchValidation(
                ...
                suggested_improvements=["Review search strategy", "Try different search terms"]
            )
    
    def suggest_search_improvements(self, question: str, current_results: str, 
                                  validation: SearchValidation) -> List[str]:
        """
        ...
        if validation.overall_score >= 0.8:
            return ["Search results are already high quality - consider adding more sources for comprehensiveness"]
        
        suggestions = []
        
        if validation.relevance_score < 0.7:
            suggestions.append("Refine search terms to be more specific to the research question")
            suggestions.append("Try alternative phrasings of the research question")
        
        if validation.quality_score < 0.7:
            suggestions.append("Look for more authoritative sources (government, academic institutions)")
            suggestions.append("Search for recent publications and reports")
        
        if validation.credibility_score < 0.7:
            suggestions.append("Focus on peer-reviewed sources and established organizations")
            suggestions.append("Cross-reference information across multiple reliable sources")
        
        if validation.completeness_score < 0.7:
            suggestions.append("Expand search to cover more aspects of the research question")
